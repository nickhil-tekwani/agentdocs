...
byId("propose").addEventListener("click", async () => {
  try {
    clearError();
    const result = await request(`/v1/workspaces/${workspaceId}/agent-runs`, { intent: byId("intent").value, targetPath, instruction: byId("instruction").value });
...
byId("publish").addEventListener("click", async () => {
  ...
  try {
    clearError();
    const result = await request(`/v1/workspaces/${workspaceId}/publish`, { proposalId, branch: byId("branch").value, message: byId("message").value });
    byId("published").textContent = `Published ${result.commitSha.slice(0, 8)} on ${result.branch}`;
    byId("publish").disabled = true;
  } catch (error) { showError(error); }
});

function fileButton(path) { const button = document.createElement("button"); button.className = "file"; button.textContent = path; button.addEventListener("click", () => { targetPath = path; document.querySelectorAll(".file").forEach((item) => item.classList.toggle("active", item === button)); }); return button; }
async function request(path, body) { return fetch(path, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) }).then(check); }
